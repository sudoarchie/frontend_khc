...
export default function AddStudent() {
  ...
  async function FetchGrade(selectedCurriculumId: string) {
    ...
    setGrade(data.data.data.data);
  }

  const mutation = useMutation({
    mutationFn: async (data: Inputs) => {
      const response = await axios({
        url: `${process.env.NEXT_PUBLIC_BASE_URL}/student/signup`,
        method: "post",
        data: data,
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 w-full sm:max-w-[900px] mt-16">
          ...
          >
            ...
            >
              {curriculum?.map((info: { id: string; name: string }) => (
          ...
          >
            ...
            <Select label="Grade" {...register("gradeId", { required: true })}>
              {grade.map((info: { id: string; name: string }, index) => (
          ...
          >
            ...
            >
              {country.map((info, index) => (
                <MenuItem value={info.code} key={index}>
                  {info.name}
                </MenuItem>
              ))}
        ...
        <Button
          variant="contained"
          className="w-full sm:max-w-[860px] mt-5 bg-primary"
          type="submit"
          disabled={mutation.isPending}
        >
          {mutation.isPending ? "Please wait..." : "Create"}
        </Button>
      </form>
    </div>
  );
